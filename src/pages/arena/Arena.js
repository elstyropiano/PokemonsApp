...
const Arena = () => {
  const { statsFromDb, arenaMember, setArenaMember } = useContext(Context)
  ...
  const handleButton = () => {
    ...
    const pokemonOne = 1
    const pokemonTwo = 1
    if (pokemonOne === pokemonTwo) setWinner([arenaMember[0], arenaMember[1]])
  ...
  const postDataOnServer = async data => {
    await axios
      ...
      .then(response => console.log(response.data, 'to wyslano na serwer'))
